...
export function ChessBoard({ pieces, selectedPiece, onPieceClick, isMyTable, lastMove }: ChessBoardProps) {
  ...
  /** 
   * 单个棋格的宽度
   * 计算公式：棋盘宽度 ÷ 列数(8列) + 1.1像素（放大）
   */
  const cellWidth = boardWidth / BOARD_COLS + 1.1;
   ...
   * 即左边框的宽度，向左移动 8 像素
   */
  const boardStartX = finalImageWidth * BORDER_LEFT_RATIO - 8;
          ...
          >
            ...
            <div style={{ zIndex: 10, position: 'absolute', inset: 0 }}>
              {pieces.map((piece, index) => {
                ...
                // 棋子显示尺寸为格子的93%
                const pieceSize = Math.min(cellWidth, cellHeight) * 0.93;
              ...
              {Array.from({ length: BOARD_ROWS + 1 }).map((_, row) => (
                <div
                  ...
                  style={{
                    ...
                    width: `${boardWidth + BOARD_COLS + 0.1 * BOARD_COLS}px`,
                    height: '1px',
                    backgroundColor: 'rgba(0, 200, 0, 0.7)',
                  }}
                />
